...
class TicTacToeboard extends React.Component{
    ...
    render(){
        ...
        if (
          ...
          if (win) {
             ...
             console.log(temp,'wintemp') 
            status = "Winner is : " + temp;
          } else {
              let temp1=this.state.next=='X'?this.state.player1:this.state.player2;  
              console.log(temp1,"nexttemp")
        ...
        return (
            <div>{status}
            {/* {console.log(status)} */ }
            <div id="boarddesign">{this.state.cells.map((value, index) => (
    <CellBoard id={index} value={value}onWinner={win ?win.winnerArray:null} onCellClick={this.handleCellClick} />
  ))}
  ...
  <div>
   <b> Player 1</b><br></br>
   <input type="text" id="player1" onChange={(e)=>{
       this.setState({
           player1:e.target.value
       })
   }}></input><br></br>
    <b> Player 2</b><br></br>
   <input type="text" id="player2" onChange={(e)=>{
       this.setState({
